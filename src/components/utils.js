export const products = [
  {
    name: "Aloevera",
    benifit:   "hydrates, soothes sunburn, reduces inflammation, promotes healing, and refreshes the skin",
    path: "Shop_list/Aloevera/Aloevera.png",
    ingredients:
      "soap base, aloevera, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E),  rose water, aloevera gel, fragrance",
  },
  {
    name: "Besan",
    path: "Shop_list/Besan/Besan (1).png",
    benifit:  " cleanses, exfoliates, brightens, fights acne, and balances oil production for clear, glowing skin" ,
    ingredients:
      " soap base, besan flour, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Camphor",
    path: "Shop_list/Camphor/Camphor.png",
    benifit: "refreshes, soothes itching, treats skin infections, improves circulation, and invigorates with a cooling sensation"  ,
    ingredients:
      "soap base, camphor powder, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel",
  },
  {
    name: "Carrot",
    path: "Shop_list/Carrot/carrot.png",
    benifit: 'brightens complexion, evens skin tone, fights aging, moisturizes, and nourishes with vitamins'  ,
    ingredients:
      "soap base, carrot puree, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Chandan",
    path: "Shop_list/Chandan/Chandan.png",
    benifit:  'calms irritated skin, evens tone, fights bacteria, reduces acne, and imparts a subtle fragrance' ,
    ingredients:
      "soap base, chandan powder, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Charcoal",
    path: "Shop_list/Charcoal/Charcoal.png",
    benifit:  'detoxifies, unclogs pores, absorbs excess oil, exfoliates, and clarifies skin for a fresh complexion.' ,
    ingredients:
      " soap base, coffee powder, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel ",
  },
  {
    name: "Coffee",
    path: "Shop_list/Coffee/Coffee.png",
    benifit: 'exfoliates, reduces cellulite, brightens skin, fights odor, and energizes with a rich aroma' ,
    ingredients:
      " soap base, coffee powder, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel",
  },
  {
    name: "Cucumber",
    path: "Shop_list/Cucumber/Cucumber.png",
    benifit:  'hydrates, soothes irritation, reduces puffiness, brightens complexion, and refreshes with a crisp scent.' ,
    ingredients:
      "soap base, cucumber puree,rice flour, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ",
  },
  {
    name: "Kesuda",
    path: "Shop_list/Kesuda/Kesuda.png",
    benifit:  'soothes skin, reduces inflammation, heals wounds, balances oil production, and imparts a subtle floral scent.' ,
    ingredients:
      " soap base, kesuda flowers, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Lavender Soap",
    path: "Shop_list/Lavender/Lavender.png",
    benifit:  'relaxes, soothes, cleanses, reduces stress, and promotes calmness with a floral scent.' ,
    ingredients:
      " soap base, lavender essential oil, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Lemon",
    path: "Shop_list/Lemon/Lemon.png",
    benifit:   'brightens complexion, clarifies skin, fights acne, reduces excess oil, and refreshes with a citrus scent.',
    ingredients:
      "soap base, lemon peel, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Multani Mitti",
    path: "Shop_list/Multani mitti/Mulatani mitti.png",
    benifit: 'purifies, detoxifies, absorbs excess oil, tightens pores, and promotes clear, healthy skin'  ,
    ingredients:
      "soap base, multanimitti powder, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Neem",
    path: "Shop_list/Neem/Neem.png",
    benifit:'cleanses, fights acne, soothes skin, reduces inflammation, and moisturizes.'   ,
    ingredients:
      "soap base, natural neem, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Orange Peel",
    path: "Shop_list/Orange-Peel/Orange peel.png",
    benifit:  ' brightens, exfoliates, fights acne, evens skin tone, and energizes with a citrus fragrance' ,
    ingredients:
      "soap base, orange peel, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Panchgavya",
    path: "Shop_list/Panchgavya/Panchgavya.jpeg",
    benifit: ' nourishes, moisturizes, purifies, balances pH, and promotes healthy skin with its blend of cow-derived ingredients.'  ,
    ingredients:
      "soap base, cow dung, urine,milk,curd,ghee,  multanimitti , mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ",
  },
  {
    name: "Papaya",
    path: "Shop_list/Papaya/Papaya.png",
    benifit: ' brightens complexion, exfoliates, evens skin tone, reduces acne, and promotes a youthful glow'  ,
    ingredients:
      "soap base, papaya puree, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Potato Soap",
    path: "Shop_list/Potato/Potato.png",
    benifit:  ' evens skin tone, reduces pigmentation, moisturizes, soothes inflammation, and promotes a radiant complexion' ,
    ingredients:
      "soap base,potato puree,rice flour, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Red Wine",
    path: "Shop_list/Red wine/Red wine (1).png",
    benifit:  'antioxidant-rich, tones skin, fights aging, improves circulation, and refreshes with a subtle wine aroma.' ,
    ingredients:
      " red wine soap base,  mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  { name: "Red Rose", path: "Shop_list/Red-Rose/Red rose.png" ,
  benifit:  'combines the cleansing properties of soap with the soothing fragrance of roses, offering a luxurious and aromatic bathing experience' ,
   ingredients:
  " soap base, red rose petals, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ",},
  {
    name: "Rice",
    path: "Shop_list/Rice/Rice.png",
    benifit: ' brightens complexion, evens skin tone, minimizes pores, exfoliates, and nourishes with vitamins.'  ,
    ingredients:
      "soap base,rice flour, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Rose",
    path: "Shop_list/Rose/rose.png",
    benifit:  ' brightens complexion, evens skin tone, minimizes pores, exfoliates, and nourishes with vitamins.' ,
    ingredients:
      " soap base, rose petals, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ",
  },
  {
    name: "Scrub",
    path: "Shop_list/Scrub/Scrub.png",
    benifit: 'exfoliates and cleanses the skin, promoting smoother and healthier- looking skin.'  ,
    ingredients:
      "soap base, papaya scrub, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Three Layer (Multani Mitti, Coffee, Besan)",
    benifit: 'exfoliates, brightens, detoxifies, and purifies skin for a refreshed complexion'  ,
    path: "Shop_list/Three layer ( multani-mitti, coffee,besan)/Three layer ( mulatani mitti, coffee, besan).png",
    ingredients:
      "soap base, besan flour, coffee powder, multani mitti, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance",
  },
  {
    name: "Tomato",
    benifit:  'brightens, tightens pores, fights acne, reduces oiliness, and refreshes with a fruity scent.' ,
    path: "Shop_list/Tomato/t2.png",
    ingredients:
      "soap base, tomato pure, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel,  fragrance, colour",
  },
  { name: "Pink Rose", path: "Shop_list/pink-Rose/Pink rose (1).png",
  benifit: 'hydrates, soothes, reduces redness, promotes healing, and imparts a delicate floral fragrance'  ,
  ingredients:
  " soap base, pink rose petals, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ", },
  {
    name: "Blue Berry",
    path: "Shop_list/⁠Blue-Barry/Blue Barry.png",
    benifit: 'nourishes, rejuvenates, brightens, fights free radicals, and promotes youthful skin.'  ,
    ingredients:
      " soap base, blueberry fruit extract, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E), rose  water, aloevera gel, fragrance ",
  },
  {
    name: "Honey",
    path: "Shop_list/Honey/Honey.png",
    benifit:  ' moisturizes, soothes, heals, balances pH, and promotes a healthy glow.' ,
    ingredients:
      " ingredients  : soap base, honey, mix oil (coconut oil, almond oil, jojoba oil, shea butter, cocoa butter, vitamin E ), rose  water, aloevera gel, fragrance",
  },
];
export const Colors ={
  mainTheme1:'#312B47',
  mainTheme2:'#8a9557',
}
export const carousel_image = [
  "https://plus.unsplash.com/premium_photo-1661333471093-e4b69add3ad0?q=80&w=2942&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  "https://good-goodal.netlify.app/assets/2-9bb13673.jpg",
  "https://images.unsplash.com/photo-1559304787-945aa4341065?q=80&w=2795&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
 "https://plus.unsplash.com/premium_photo-1679064286475-f15eca3e313d?q=80&w=2787&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
 "https://media.istockphoto.com/id/115037368/photo/pieces-of-natural-soap.jpg?s=1024x1024&w=is&k=20&c=1eOMGEFNiNuMqjBD0oxxwvvdshDiOUaSxjY-7GM9IPY=",
 "https://cdn.pixabay.com/photo/2017/05/22/07/39/soap-2333412_1280.jpg",
  "https://images.unsplash.com/photo-1474625121024-7595bfbc57ac?q=80&w=2950&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
,"https://images.unsplash.com/photo-1600857544200-b2f666a9a2ec?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
]

export  const reviews = [
  { id: 1, author: 'John Doe', content: 'Great product! Highly recommend.' },
  { id: 2, author: 'Jane Smith', content: 'Excellent quality and fast shipping.' },
  { id: 3, author: 'Alex Johnson', content: 'Very satisfied with my purchase.' },
  { id: 4, author: 'Emily Brown', content: 'Fantastic customer service!' },
  { id: 5, author: 'Michael Lee', content: 'Will definitely buy again.' },
];
export const product_price =[
  {weight:'100gm',amount:'₹89.00'},
  {weight:'80gm',amount:'₹79.00'},
  {weight:'60gm',amount:'₹69.00'},
];
export const slides = [
  {
    image: 'carousel/1.png',
    title: 'Slide 1',
    quote: 'Moisturizing formulations that will make your skin even more bright.',
  },
  {
    image: 'carousel/2.png',
    title: 'Slide 2',
    quote: 'Complete your elastic skin with the apricot collagen line.',
  },
  {
    image: 'carousel/3.png',
    title: 'Slide 3',
    quote: 'Face and body cleansing bars and shampoo bars.',
  },
  {
    image: 'carousel/4.png',
    title: 'Slide 3',
    quote: 'Cleanliness is not just a choice, its a habit. Embrace the purity of every bubble.',
  },
  {
    image: 'carousel/5.png',
    title: 'Slide 3',
    quote: 'Let the soothing suds cleanse not just your skin, but your soul too.',
  },
  {
    image: 'carousel/6.jpg',
    title: 'Slide 3',
    quote: 'Cleanliness is next to godliness.,Soap may not cure all things, but its a good start',
  },

  // Add more slides as needed
];